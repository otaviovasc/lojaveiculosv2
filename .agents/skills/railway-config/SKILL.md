...
```ts
const worker = service("scheduled-worker", {
  build: "pnpm --filter api build",
  deploy: {
    cronSchedule: "*/5 * * * *",
    restartPolicyType: "NEVER",
  },
  start: "pnpm --filter api worker:run",
});
```

Keep `cronSchedule` nested under `deploy`. A top-level `cronSchedule` may be
silently ignored by the runner. Confirm the machine-readable plan contains
`desiredGraph.resources[].deploy.cronSchedule` before treating the schedule as
managed. Set `restartPolicyType: "NEVER"` so a completed or failed cron run
does not become a permanent restart loop.
...
- Cron schedules appear under the service's `deploy` object in the
