...
| `Entry`, `RecurringEntry`                           | Use finance entries and recurrence schedules with links to vehicle, sale, lead, or service. Vehicle cost, reserve, and sell workflows create `finance_entries` with `finance_entry_links` to `vehicle_cost`, `vehicle_listing`, `vehicle_unit`, `sale`, and `sale_payment` targets. |
