...
- The current route identifier is accepted only as a resolver for the linked V2
  lead and activity
  metadata. It is not persisted on `lead_visits`.
- The CRM visits page is a real operations page, not a dialog. It supports
  today, upcoming, overdue, and completed views, creation from the active linked
  conversation, status actions, permission states, loading, empty, and error states.
