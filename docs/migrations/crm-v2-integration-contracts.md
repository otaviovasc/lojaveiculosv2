...
The runtime separates three independent concepts:

- **channel:** `whatsapp`, `instagram`, or `olx_chat`;
- **provider:** `meta_cloud`, `zapi`, or `olx`;
- **broker:** `composio` for Meta authorization/execution, or `direct` for
  ZAPI and OLX transport.
...
- Provider selection belongs to the persisted channel connection. There is no automatic
...
`crm` store entitlements plus `crm.messages.ingest`, and returns HTTP 200 with a
...
for the WhatsApp channel or professional-account id for the Instagram channel.
Instagram sender discovery and the provider subscription target remain
distinct for Facebook Login for Business.

### Conversation HTTP surface

The current conversation routes are channel-neutral. Routing resolves the
selected channel connection and persistence uses canonical threads, cycles,
attendances, and messages.

- `GET /crm/conversation-cycles`
- `GET /crm/conversation-cycles/counts`
- `GET /crm/conversation-cycles/:cycleId/messages`
- `POST /crm/conversation-cycles`
- `POST /crm/conversation-cycles/:cycleId/messages`
- `POST /crm/conversation-cycles/:cycleId/actions/read`
...
The conversation attendance is the canonical attendance record and is linked
to the V2 `leads.id`. Conversation responses and `conversation` realtime events expose
...
| `humanAttendanceStateVersion` | Positive integer or `null`                     | Conversation-monotonic version; `null` only before the first attendance transition. |
...
- An explicit manual takeover also sets `IN_HUMAN_SERVICE`. Ending the
  intervention, closing or reopening the conversation, or resuming automation sets
...
`GET /crm/whatsapp/events` as a named `conversation` event. The event contains
the complete V2 conversation projection, including the five fields above, plus the
stream event ID. Consumers must deduplicate event IDs, apply only a greater
`humanAttendanceStateVersion` for a conversation, and use the server timestamps for
...
If realtime delivery is unavailable, the persisted conversation state remains the
...
- `GET /crm/scheduled-messages`
- `POST /crm/scheduled-messages`
- `POST /crm/scheduled-messages/process-due`
- `DELETE /crm/scheduled-messages/:scheduledMessageId`
...
old Repasses conversation JSON fields.
...
store-scoped canonical conversation/message model. Instagram messages use
channel `instagram`; Instagram receipts are intentionally ignored until a verified
...
provider ad objects into conversation/message metadata or creating synthetic buyer
messages:

- `externalAdReply` and CTWA referral/context fields are normalized into
  allowlisted conversation attribution metadata.
- Meta/ZAPI notification callbacks may create or enrich a conversation with ad
...
- A buyer message that carries ad context, including the documented LID ad
  ...
  same allowlisted `conversation.adAttribution`; notification-first conversations
...
- A real phone may replace a LID placeholder only when the same `chatLid`
  ...
  LID conversation or linked lead.
- The attribution write and intervention transition are tenant/store scoped,
  audited through `crm.messages.ingest`, and idempotent under webhook retries.
...
Leading measures are attributed-conversation coverage, ad-lead first-response
...
conversation/message state and surfaces dispatch failure through logs/audit; it must
...
source vocabulary only. New code and the cutover catalog use the channel-
neutral keys above.
...
- `crm_campaigns` and `crm_campaign_recipients`
- `crm_external_bot_grants`, `crm_external_bot_action_commands`, and
  `crm_external_bot_provider_effects`
- `crm_external_bot_event_outbox`, `crm_external_bot_kill_switches`,
  `crm_external_bot_proposals`, and `crm_external_bot_policies`
- `crm_pipelines`
- `crm_pipeline_stages`
- `lead_visits`
- `leads`
...
There are no schema-only CRM sync-event placeholders. Provider receipts and
effects use their canonical provider-event and outbox tables.
