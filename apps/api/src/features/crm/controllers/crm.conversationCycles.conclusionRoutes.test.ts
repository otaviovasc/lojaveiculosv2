...
import {
  ...
  storeId,
  tenantId,
} from "./crm.conversationCycleConclusionRoutes.testSupport.js";

describe("POST CRM attendance conclusion", () => {
  it("applies and replays follow-up without moving the lead or duplicating its task", async () => {
    ...
    const request = () =>
      fixture.app.request(
        `/api/v1/crm/conversation-cycles/${fixture.originId}/attendance/conclusion`,
        {
          body: JSON.stringify({
    ...
    const cycles = await fixture.conversationRepository.listConversationCycles({
      leadId: fixture.leadId,
      limit: 10,
      offset: 0,
      storeId,
    ...
    expect(cycles.find((item) => item.id === fixture.otherId)?.status).toBe(
      "ACTIVE",
    );
  ...
  it("moves a lost lead by stage status and completes every active channel cycle", async () => {
    const fixture = await createConclusionFixture();
