...
describe("CRM scheduled-message processing", () => {
  it("claims each due message once across concurrent processor runs", async () => {
    ...
    const seeded = await seedCycle(repository, "5511999999304");
    const scheduled = await repository.createScheduledMessage({
      ...
      content: "Mensagem processada uma vez",
    });
  ...
  it("lets cancellation win before a due message is claimed", async () => {
    const repository = createMemoryCrmConversationRepository();
    const seeded = await seedCycle(repository, "5511999999305");
    const scheduled = await repository.createScheduledMessage({
      connectionId: seeded.conversationCycle.connectionId,
      recipientAddress: seeded.conversationCycle.customerPhone,
      scheduledAt: new Date("2030-01-01T10:00:00.000Z"),
