...
import type {
  RawCrmConversationFixture,
  RawCrmConversationScope,
} from "./drizzleCrmConversationConsistency.rawDbTestTypes.js";
...
export async function seedRawCrmConversationFixture(
  transaction: DrizzleCrmClient,
): Promise<RawCrmConversationFixture> {
...
export function rawCrmMessage(
  current: RawCrmConversationScope,
  overrides: Partial<IngestCrmMessageInput> = {},
): IngestCrmMessageInput {
  const input: IngestCrmMessageInput = {
    customerDisplayName: "Raw CRM buyer",
    customerPhone: `5511${randomUUID().replaceAll("-", "").slice(0, 10)}`,
    channel: "WHATSAPP",
...
export async function seedRawCrmMessage(
  repository: CrmConversationRepository,
  current: RawCrmConversationScope,
  overrides: Partial<IngestCrmMessageInput> = {},
) {
  return repository.ingestMessage(rawCrmMessage(current, overrides));
}

export async function assertRawCrmConversationDuplicateAndCas(
  repository: CrmConversationRepository,
  current: RawCrmConversationScope,
) {
  const customerPhone = `55000${Date.now().toString().slice(-8)}`;
  const echo = rawCrmMessage(current, {
    customerPhone,
  ...
  const inbound = await seedRawCrmMessage(repository, current, {
    customerPhone,
...
function scope(storeId: string, tenantId: string): RawCrmConversationScope {
  return {
