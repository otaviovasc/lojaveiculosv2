...
describe.skipIf(!runRawDb)(
  "CRM campaign reply failure Postgres consistency",
  () => {
    it("keeps secondary failure visible when initial bookkeeping arrives late", async () => {
      await withRawCrmTransaction(async (transaction) => {
        ...
        const reply: ClaimCrmCampaignReplyInput = {
        ...
        expect(await claimCrmCampaignReply(transaction, reply, true)).toEqual(
          expect.objectContaining({ repliedCount: 1, scheduledCount: 2 }),
        );
        ...
        const [persistedRecipient] = await transaction
          .select({
            errorMessage: schema.crmCampaignRecipients.errorMessage,
        ...
        expect(persistedRecipient).toEqual({
          errorMessage: "secondary provider unavailable",
          initialSentAt: new Date("2030-01-01T10:01:00.000Z"),
          status: "failed",
        });
      });
    });
