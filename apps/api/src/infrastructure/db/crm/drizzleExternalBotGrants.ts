...
export function createExternalBotGrantStore(
  ...
  return {
    issue: async (grant) => {
      ...
      const providerRows = await input.db
        .execute(sql`select provider from provider_connections
        where id=${grant.connectionId}::uuid and tenant_id=${grant.tenantId}::uuid and store_id=${grant.storeId}::uuid limit 1`);
      const provider = (providerRows as unknown as ExternalBotRow[])[0]
        ?.provider;
      if (!provider) {
    ...
    consume: async (grant) => {
      const rows = await input.db.execute(sql`
        ...
        where token_digest = ${digest(grant.token)} and tenant_id = ${grant.tenantId}::uuid
          ...
          and exists (select 1 from provider_connections connection where connection.id=provider_connection_id and connection.provider=${grant.provider})
        returning id`);
