...
export async function claimDrizzleCrmRetentionScopes(
  ...
  return db.transaction(async (tx) => {
    await tx.execute(sql`
      ...
      select store.tenant_id, store.id, ${input.now}
      from stores store
    ...
    const rows = await tx.execute(sql`
      with candidates as (
        ...
        where scope.next_run_at <= ${input.now}
          and (scope.lease_expires_at is null or scope.lease_expires_at <= ${input.now})
...
export async function completeDrizzleCrmRetentionScope(
  ...
  const rows = await db.execute(sql`
    ...
    set cursor = ${input.cursor ?? null},
        ...
        last_completed_at = case when ${input.succeeded} then ${input.now} else last_completed_at end,
        last_failed_at = case when ${input.succeeded} then last_failed_at else ${input.now} end,
        updated_at = ${input.now}
