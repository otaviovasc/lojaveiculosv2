...
export function resolveComposioCrmCredentials(
  ...
  assertComposioConnection(connection);
  ...
  return {
    ...
    apiKey,
    channel: connection.channel as "instagram" | "whatsapp",
    connectedAccountId,
    graphVersion,
    provider: "meta_cloud",
...
function assertComposioConnection(connection: CrmConnection) {
  if (
    ...
    throw new CrmMessagingGatewayError(
      `Unsupported Composio CRM connection: ${connection.channel}/${connection.provider}/${connection.broker}`,
    );
  }
}
