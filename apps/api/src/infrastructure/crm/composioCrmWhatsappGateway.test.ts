...
describe("Composio CRM messaging gateway", () => {
  it("sends official WhatsApp text through the authenticated proxy", async () => {
    ...
    const gateway = createComposioCrmWhatsappGateway(env, fetchImpl);

    const result = await gateway.sendText(
      createConnection("composio_whatsapp"),
      { phone: "5511999999999", text: "Olá" },
    );
  ...
  it("uses the Instagram messaging payload for customer-initiated sessions", async () => {
    const fetchImpl = vi.fn<typeof fetch>(async () =>
      Response.json({
    ...
    await gateway.sendText(createConnection("composio_instagram"), {
  ...
  it("retries only explicit 429 responses with bounded Retry-After", async () => {
    ...
    const result = await executeComposioProxy(
      resolveComposioCrmCredentials(createConnection("composio_whatsapp"), env),
      { body: {}, endpoint: "https://graph.facebook.com/test/messages" },
      fetchImpl,
      { maxRetries: 1, sleep },
    );
  ...
  it("fails closed for unsupported Instagram capabilities", async () => {
    ...
    const connection = createConnection("composio_instagram");
    ...
    ).rejects.toBeInstanceOf(CrmWhatsappGatewayError);
    await expect(
  ...
  it("rejects raw provider secrets stored in connection metadata", () => {
    const connection = createConnection("composio_whatsapp", {
      metadata: {
