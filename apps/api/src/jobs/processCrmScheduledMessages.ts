...
async function main(): Promise<void> {
  ...
  try {
    ...
    const dueAt = parseOptionalDate("CRM_SCHEDULE_DUE_AT") ?? new Date();
    const perScopeLimit =
      parseOptionalPositiveInt("CRM_SCHEDULE_BATCH_SIZE") ?? 25;
    const scopeLimit =
      parseOptionalPositiveInt("CRM_SCHEDULE_SCOPE_LIMIT") ?? 100;
    ...
    logger.info("crm.schedule.worker.finished", {
      ...result,
...
async function processDueScheduledMessages(input: {
  audit?: AuditSink;
  dueAt: Date;
  ...
  const scopes = await input.services.listDueCrmScheduledMessageScopes(
    discoveryContext,
  ...
  for (const scope of scopes) {
    ...
    const result = await input.services.processDueCrmScheduledMessages(
      context,
...
void main().catch((error) => {
  logger.error("job.crm_schedule.failed", {
