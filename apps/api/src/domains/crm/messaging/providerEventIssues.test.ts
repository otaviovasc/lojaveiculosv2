...
describe("whatsappWebhookEventIssues", () => {
  ...
  it.each([
    { eventType: "crm.provider.zapi.delivery", payload: baseEvent.payload },
  ...
  it.each([
    ...
    expect(readZapiWebhookType(`crm.provider.zapi.${type}`)).toBe(type);
  });

  it("rejects non-ZAPI and unknown ZAPI event types", () => {
    expect(readZapiWebhookType("other.received")).toBeNull();
    expect(readZapiWebhookType("crm.provider.zapi.unknown")).toBeNull();
  });
});
