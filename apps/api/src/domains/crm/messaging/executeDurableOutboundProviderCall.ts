...
import type { ProviderSentMessage } from "./outboundMessageTypes.js";
import type {
...
export async function executeDurableOutboundProviderCall(
  ...
  const repository = getCrmOutboundIntentRepository(ports);
...
export async function completeDurableOutboundProviderCall(
  ...
  await getCrmOutboundIntentRepository(ports).complete(input);
}
