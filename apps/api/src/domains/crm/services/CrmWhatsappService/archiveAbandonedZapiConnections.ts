...
export async function archiveAbandonedZapiConnections(
  ...
  const recoveryPayloadsPurged = ports.crmOutboundIntentRepository
    ? await ports.crmOutboundIntentRepository.purgeExpiredRecoveryPayloads({
        limit,
        now,
      })
    : 0;
  ...
  for (const connection of archived) {
    await auditCrmServiceEvent(context, {
      action: "crm.provider.zapi.connection.abandoned.archive",
  ...
  logCrmServiceEvent(context, "crm.provider.zapi.cleanup.completed", {
