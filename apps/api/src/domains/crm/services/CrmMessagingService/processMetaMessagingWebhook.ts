...
export async function processMetaMessagingWebhook(
  ...
  logCrmServiceEvent(context, "crm.messaging.webhook.meta.received", {
...
async function ingestMessage(
  ...
  const persisted = await runCrmTransaction(ports, async (transactionPorts) => {
    const { customerPhone, channel, lead, repository } =
  ...
  const conversationCycle = persisted.conversationCycle;
