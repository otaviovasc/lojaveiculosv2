...
import type { persistZapiCanonicalInbound } from "./persistZapiCanonicalInbound.js";
...
export type PersistWhatsappWebhookInput = {
...
export async function persistWhatsappWebhook(
  ...
  input: PersistWhatsappWebhookInput,
  ports: CrmServicePorts,
  persistInbound: typeof persistZapiCanonicalInbound,
) {
  const { attribution, connection, detectedAt, media, parsed, profilePhoto } =
    input;
  const directHumanOutbound = isDirectHumanOutboundEcho(parsed);
  ...
  return runCrmTransaction(ports, async (transactionPorts) => {
    if (!parsed.fromMe) {
      ...
      const canonical = await persistInbound(transactionPorts, {
        attribution,
        connection,
        lead,
        media,
