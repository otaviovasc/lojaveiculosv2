...
describe("CRM WhatsApp automatic outbound assignment", () => {
  it.each([
    ["HUMAN", "human_crm", true],
    ["HUMAN", "human_whatsapp", false],
    ["AI", "bot_api", false],
  ...
  it("preserves a concurrent assignee after a CAS conflict", async () => {
    const initial = createTestCrmWhatsappSession({
      id: "session-race",
      leadId: "lead-race",
    });
    const concurrent = createTestCrmWhatsappSession({
    ...
    const result = await applyWhatsappSessionAssignment({
    ...
    expect(result).toMatchObject({
      result: "superseded",
      session: { assignedUserId: "other-user", revision: 1 },
    });
  ...
  it("records attempted and succeeded evidence for an applied assignment", async () => {
    ...
    const ports = transactionalPorts({ updateSession } as never);
  ...
  it("records no-change evidence without stealing an existing assignment", async () => {
    ...
    await expect(
      ...
      assignment: {
        result: "superseded",
        session: { assignedUserId: "other-user" },
      },
    });
...
function activeHumanSession(
  overrides: Parameters<typeof createTestCrmWhatsappSession>[0] = {},
) {
  return createTestCrmWhatsappSession({
...
function transactionalPorts(
  crmWhatsappRepository: NonNullable<CrmServicePorts["crmWhatsappRepository"]>,
) {
...
function automaticInput(
  session: ReturnType<typeof createTestCrmWhatsappSession>,
  ...
  return {
    ...
    scope: { storeId: session.storeId, tenantId: session.tenantId },
