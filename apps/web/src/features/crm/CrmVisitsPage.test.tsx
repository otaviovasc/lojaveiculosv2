...
describe("CrmVisitsPage", () => {
  afterEach(() => {
    cleanup();
    vi.clearAllMocks();
  });

  it("creates a visit from the active linked WhatsApp cycle", async () => {
...
function renderPage(
  api: CrmVisitsApi,
  activeSession: CrmConversationCycle | null = createSession(),
  canManage = true,
  listVehicles?: ComponentProps<typeof CrmVisitsPage>["listVehicles"],
) {
  render(
    <CrmVisitsPage
...
function createSession(): CrmConversationCycle {
  return {
    customerDisplayName: "Lead Visita",
