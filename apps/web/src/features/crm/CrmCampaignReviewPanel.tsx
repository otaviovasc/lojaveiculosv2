...
import {
  ...
  ReviewStat,
} from "./CrmCampaignReviewPanelParts";
...
export function CampaignReviewPanel({
  ...
  return (
    <section className="crm-campaign-panel crm-campaign-review-panel">
      <header>
        <h3>
      ...
      <dl className="crm-campaign-review">
        <ReviewStat
          active={filter === "all"}
          label="Incluidos"
      ...
      <pre>{preview}</pre>
      <div className="crm-campaign-review-tools">
      ...
      {summary.blockedIncluded ? (
        <p className="crm-campaign-error">
          Corrija ou remova destinatarios bloqueados antes de criar a campanha.
        </p>
      ) : null}
      {lastResult ? <p className="crm-campaign-success">{lastResult}</p> : null}
    </section>
  );
}
