...
import type { CrmVehicleOption, CrmVehicleQuery } from "./crmConversationTypes";

export function useCrmVehicleInventory() {
  ...
  return useCallback(
    async (
      ...
      return result.items.map(toCrmVehicleOption);
    },
    [inventoryApi],
  );
}

function toCrmVehicleOption(item: InventoryListingSummary): CrmVehicleOption {
