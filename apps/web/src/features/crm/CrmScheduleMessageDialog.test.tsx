...
describe("CrmScheduleMessageDialog", () => {
  it("uses the shared modal layer outside the embedded page", async () => {
    const callbacks = createCallbacks();
    render(<CrmScheduleMessageDialog {...baseProps} {...callbacks} />);

    const dialog = await screen.findByRole("dialog", {
      name: "Agendamentos WhatsApp",
    });
    expect(dialog).toHaveClass("crm-action-panel");
  ...
  it("preserves the non-modal embedded region", () => {
    ...
    ).toHaveClass("crm-action-embedded");
