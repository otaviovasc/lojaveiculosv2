...
describe("CrmSchedulesPage", () => {
  afterEach(() => cleanup());

  it("lists store-wide schedules and applies status/cycle filters", async () => {
    ...
    await waitFor(() =>
      expect(callbacks.onList).toHaveBeenLastCalledWith({
        connectionId: "24000000-0000-4000-8000-000000000101",
        limit: 100,
        cycleId: "34000000-0000-4000-8000-000000000001",
      }),
    );
  });

  it("prefills the active cycle for one-off schedule creation", async () => {
    const user = userEvent.setup();
...
function renderPage(
  overrides: Partial<Parameters<typeof CrmSchedulesPage>[0]> = {},
) {
  const conversationCycles = overrides.conversationCycles ?? [createSession()];
  const callbacks = {
  ...
  render(
    <CrmSchedulesPage
      activeSession={conversationCycles[0] ?? null}
      canCancel={true}
      canCreate={true}
      canProcess={true}
      canRead={true}
...
function createSession(
  ...
  return {
    customerDisplayName: input.customerDisplayName ?? "Bruno",
    customerPhone: input.customerPhone ?? "5511999990000",
    channel: "whatsapp",
...
function createScheduledMessage(
  input: Partial<CrmScheduledMessage> = {},
): CrmScheduledMessage {
  return {
    cancelledAt: null,
    connectionId: "24000000-0000-4000-8000-000000000101",
    ...
    content: input.content ?? "Ola futuro",
