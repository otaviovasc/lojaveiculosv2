...
describe("CrmProviderEventIssuesPanel", () => {
  afterEach(() => {
    cleanup();
    vi.restoreAllMocks();
  });
  ...
  it("lists failed events and retries them", async () => {
    ...
    render(<CrmProviderEventIssuesPanel api={api} canRetry={true} />);
  ...
  it("shows failed events without retry actions for read-only users", async () => {
    ...
    render(<CrmProviderEventIssuesPanel api={api} canRetry={false} />);
        ...
        events: [
          {
            ...
            eventType: "crm.messaging.zapi.connected",
            id: "event_1",
            processedAt: null,
            provider: "zapi",
