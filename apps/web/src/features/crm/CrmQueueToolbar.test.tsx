...
import { formatCrmMemberRole } from "./CrmQueueToolbarParts";
import type {
  CrmAssignableMember,
...
describe("CrmQueueToolbar", () => {
  ...
  it("uses dealership role labels in the assignee hierarchy", () => {
    expect(formatCrmMemberRole("owner")).toBe("Dono");
    expect(formatCrmMemberRole("salesman")).toBe("Vendedor");
    expect(formatCrmMemberRole("supervisor")).toBe("Supervisor");
  });
...
function createConnections(): CrmProviderConnection[] {
  return [
