...
function InventoryListingCard({
  ...
  return (
    <motion.article
      ...
      className="glass-panel-branded flex flex-col h-full group cursor-pointer relative"
    ...
    >
      <div className="relative flex aspect-[16/10] items-center justify-center overflow-hidden bg-app-elevated border-b border-line/30">
        {item.primaryMediaUrl ? (
          ...
          <CarFront aria-hidden="true" className="size-14 text-muted/50" />
        )}
        <div className="absolute left-3 top-3">
          <StatusPill status={listing.status} />
        </div>
        <div className="absolute right-3 top-3 rounded-full bg-panel/90 backdrop-blur-md px-3 py-1 text-xs font-black text-app-text border border-line/30 shadow-sm">
      ...
      <div className="flex flex-col gap-4 p-5 flex-grow">
        <div className="min-w-0">
          <h3 className="truncate text-lg font-black text-app-text group-hover:text-accent transition-colors">
            {listing.title}
          </h3>
        ...
        <div className="grid grid-cols-2 gap-3 border-y border-line/45 py-3.5 my-1">
          <MetaItem
            icon={
              <CalendarDays
                aria-hidden="true"
          ...
          <MetaItem
            icon={
              <ImageIcon aria-hidden="true" className="size-4 text-pink-500" />
            }
            label="Estoque"
        ...
        <div
          className="grid grid-cols-3 gap-2 mt-auto pt-2 relative"
        ...
        >
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute bottom-14 right-0 z-30 w-48 rounded-xl border border-line bg-panel p-1.5 shadow-2xl">
                <button
                  onClick={() => {
                    setMenuOpen(false);
                    onAction?.("template", item);
                  }}
                  className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-black text-app-text hover:bg-line/25 cursor-pointer"
                >
