...
function buildGlowVars(
  ...
  for (let i = 0; i < opacities.length; i++) {
    vars[`--glow-color${keys[i]}`] = `${hslFn}(${base} / ${Math.min(
      (opacities[i] ?? 100) * intensity,
      100,
    )}%)`;
  }
  return vars;
...
function buildGradientVars(colors: string[]): Record<string, string> {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    vars[GRADIENT_KEYS[i] ?? ""] = `radial-gradient(at ${
      GRADIENT_POSITIONS[i]
    }, ${c} 0px, transparent 50%)`;
  }
...
function animateValue({
  ...
  onUpdate,
  onEnd,
}: AnimateValueOptions): AnimationHandle {
  ...
  useEffect(() => {
    ...
    const animations = [
      animateValue({
        duration: 500,
        onUpdate: (v) =>
          card.style.setProperty("--edge-proximity", v.toString()),
      }),
      animateValue({
        ease: easeInCubic,
        duration: 1500,
